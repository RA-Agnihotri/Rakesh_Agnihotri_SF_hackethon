...
import { Database, CheckCircle2, DollarSign, Zap, Bot, Send, User, Sparkles, X } from 'lucide-react';
  ...
  return (
    <div className="flex gap-4 h-[calc(100vh-8rem)]">
      ...
      <div className="w-[380px] flex-shrink-0 flex flex-col bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700">
        ...
        <div className="border-t border-gray-200 dark:border-slate-700 p-3"><div className="flex items-center bg-gray-50 dark:bg-slate-700 rounded-lg border border-gray-200 dark:border-slate-600 focus-within:ring-2 focus-within:ring-green-500"><input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && handleAsk()} placeholder="Ask about pricing..." className="flex-1 px-3 py-2.5 bg-transparent text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none" disabled={isLoading} /><button onClick={() => handleAsk()} disabled={!input.trim() || isLoading} className="p-2.5 text-green-600 hover:text-green-700 disabled:opacity-30"><Send className="w-4 h-4" /></button></div></div>
      </div>
    </div>
  );
}
